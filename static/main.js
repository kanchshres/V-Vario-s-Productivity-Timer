  ...
  function updateClock() {
    ...
    if (t.total == 0) {
        ...
        let randomInt = Math.floor(Math.random() * listOfTips.length);
          //Displays Tip + Image
          if (breakType == "start") {
            document.getElementById('breakdiv').innerHTML = "<img src=' "+listOfTips[randomInt].image+"'>" + listOfTips[randomInt].tip;
            listOfTips.splice(randomInt,1) //removes tip so won't display again
          }
          else if (breakType == "quickBreak" || breakType == "longBreak") {
